...
function Field({ label, value, onChange, error, type = 'text', placeholder, max, required = false }: { label: string; value: string; onChange?: (value: string) => void; error?: string; type?: string; placeholder?: string; max?: string; required?: boolean }) {
  return <label className="block text-[10px] uppercase tracking-[0.25em] text-charcoal/55">{label}<input type={type} value={value} max={max} required={required} aria-required={required} placeholder={placeholder} readOnly={!onChange} aria-invalid={Boolean(error)} onChange={(event) => onChange?.(event.target.value)} className={`${inputClass} ${error ? 'border-red-500 text-red-700' : ''}`} />{error ? <span role="alert" className="mt-2 block text-[10px] normal-case tracking-normal text-red-600">{error}</span> : null}</label>;
}
...
function ProfilePageContent() {
  ...
  const [passwordStep, setPasswordStep] = useState<'idle' | 'details' | 'otp'>('idle');
  ...
  const validate = () => {
    ...
    else if (form.dob >= new Date().toISOString().slice(0, 10)) next.dob = 'Enter a valid date of birth.';
  ...
  const save = async () => {
    ...
    try {
      ...
      const profile = data.user as Profile; const next = toForm(profile); setUser(profile); setForm(next); setSaved(next); window.localStorage.setItem('rk_auth_user', JSON.stringify(profile)); setToast({ type: 'success', text: 'Your profile has been saved.' });
  ...
  const deleteAccount = async () => {
    ...
    window.localStorage.removeItem('rk_access_token'); window.localStorage.removeItem('rk_auth_token'); window.localStorage.removeItem('rk_auth_user'); window.location.assign('/account');
  };
  ...
  const requestPasswordChange = async () => {
    if (newPassword.length < 8) { setToast({ type: 'error', text: 'Password must be at least 8 characters.' }); return; }
    if (newPassword !== confirmPassword) { setToast({ type: 'error', text: 'New passwords do not match.' }); return; }
    setPasswordBusy(true);
    try {
      const response = await fetch(`${apiBaseUrl}/api/auth/profile/password/request`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` }, body: JSON.stringify({ password: newPassword, confirmPassword }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? 'Could not send verification code.');
      setPasswordStep('otp'); setToast({ type: 'success', text: 'Verification code sent to your registered email.' });
    } catch (error) { setToast({ type: 'error', text: error instanceof Error ? error.message : 'Could not send verification code.' }); }
    finally { setPasswordBusy(false); }
  };

  const verifyPasswordChange = async () => {
    if (passwordOtp.length !== 6) { setToast({ type: 'error', text: 'Enter the six-digit verification code.' }); return; }
    setPasswordBusy(true);
    try {
      const response = await fetch(`${apiBaseUrl}/api/auth/profile/password/verify`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token()}` }, body: JSON.stringify({ otp: passwordOtp, password: newPassword, confirmPassword }) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error ?? 'The verification code is invalid or expired.');
      setPasswordStep('idle'); setNewPassword(''); setConfirmPassword(''); setPasswordOtp(''); setToast({ type: 'success', text: 'Your password has been changed.' });
    } catch (error) { setToast({ type: 'error', text: error instanceof Error ? error.message : 'Could not change your password.' }); }
    finally { setPasswordBusy(false); }
  };
  ...
  return <main className="account-profile-page min-h-screen bg-[radial-gradient(circle_at_top_right,_rgba(199,166,107,0.18),_transparent_35%),linear-gradient(135deg,#f8f6f2_0%,#f2eee7_100%)] px-5 py-8 text-charcoal md:px-10 lg:px-16">
    ...
    <div className="mx-auto mt-16 max-w-6xl"><p className="text-[10px] uppercase tracking-[0.35em] text-gold">Personal space</p><div className="mt-3 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><h1 className="font-display text-6xl leading-none md:text-8xl">My profile.</h1><p className="mt-5 max-w-xl text-sm leading-7 text-charcoal/60">A quieter place to keep your RK experience personal.</p></div><div className="flex flex-wrap items-center gap-4">{user.role === 'customer' ? <Link href="/account/orders" className="rounded-full border border-gold/70 px-5 py-3 text-[10px] uppercase tracking-[0.2em] text-gold transition hover:bg-gold hover:text-ink">My orders</Link> : user.role === 'admin' ? <Link href="/admin" className="rounded-full border border-gold/70 px-5 py-3 text-[10px] uppercase tracking-[0.2em] text-gold transition hover:bg-gold hover:text-ink">Admin dashboard</Link> : user.role === 'staff' ? <Link href="/staff" className="rounded-full border border-gold/70 px-5 py-3 text-[10px] uppercase tracking-[0.2em] text-gold transition hover:bg-gold hover:text-ink">Staff dashboard</Link> : null}{dirty ? <p className="text-xs text-charcoal/55">You have unsaved changes.</p> : null}</div></div></div>
    <div className="mx-auto mt-12 grid max-w-6xl gap-6 lg:grid-cols-[0.75fr_1.25fr]">
      <section className={`${cardClass} h-fit rounded-[2rem]`}><p className="text-[10px] uppercase tracking-[0.3em] text-charcoal/45">Profile picture</p><div className="mt-8 flex items-center gap-5"><div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full bg-charcoal text-2xl text-ivory">{form.profileImage ? <img src={form.profileImage} alt="Profile preview" className="h-full w-full object-cover" /> : initials}</div><div className="min-w-0"><p className="truncate text-sm font-medium">{form.firstName || form.lastName ? `${form.firstName} ${form.lastName}`.trim() : 'Your full name'}</p><p className="mt-1 text-[10px] uppercase tracking-[.18em] text-gold">{user.role}</p><p className="mt-1 truncate text-xs text-charcoal/50">{form.email}</p><p className="mt-2 text-xs text-charcoal/45">JPG, PNG or WebP · 3 MB max</p><div className="mt-4 flex gap-4 text-[10px] uppercase tracking-[0.2em]"><label className="cursor-pointer text-gold hover:text-charcoal">{form.profileImage ? 'Change' : 'Upload'}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseImage} className="hidden" /></label>{form.profileImage ? <button type="button" onClick={() => setValue('profileImage', null)} className="text-charcoal/45 hover:text-red-600">Remove</button> : null}</div></div></div></section>
      <section className={`${cardClass} rounded-[2rem]`}><div className="flex items-end justify-between"><div><p className="text-[10px] uppercase tracking-[0.3em] text-charcoal/45">Personal information</p><h2 className="mt-3 font-display text-4xl">Tell us about you.</h2><p className="mt-3 text-xs text-charcoal/50">Phone number, date of birth, and gender are required to save your profile.</p></div></div><div className="mt-10 grid gap-7 md:grid-cols-2"><Field label="First name" value={form.firstName} onChange={(value) => setValue('firstName', value)} /><Field label="Last name" value={form.lastName} onChange={(value) => setValue('lastName', value)} /><Field label="Username" value={form.username} onChange={(value) => setValue('username', value)} error={errors.username} /><Field label="Email" value={form.email} /><Field label="Phone number · required" required value={form.phone} onChange={(value) => setValue('phone', value)} error={errors.phone} placeholder="+91 00000 00000" /><Field label="Date of birth · required" required type="date" value={form.dob} onChange={(value) => setValue('dob', value)} error={errors.dob} max={new Date().toISOString().slice(0, 10)} /><label className="block text-[10px] uppercase tracking-[0.25em] text-charcoal/55">Gender · required<select required value={form.gender} aria-invalid={Boolean(errors.gender)} onChange={(event) => { setValue('gender', event.target.value); setErrors((current) => ({ ...current, gender: '' })); }} className={`${inputClass} ${errors.gender ? 'border-red-500 text-red-700' : ''}`}><option value="">Select</option><option value="male">Male</option><option value="female">Female</option><option value="prefer-not-to-say">Prefer not to say</option></select>{errors.gender ? <span role="alert" className="mt-2 block text-[10px] normal-case tracking-normal text-red-600">{errors.gender}</span> : null}</label><label className="block text-[10px] uppercase tracking-[0.25em] text-charcoal/55">Region<select value={form.region} onChange={(event) => { const region = event.target.value; setForm((current) => ({ ...current, region, currency: regionCurrency[region] })); }} className={inputClass}><option value="asia-india">Asia (India)</option><option value="us">US</option><option value="europe">Europe</option><option value="anywhere-else">Anywhere else</option></select></label><label className="block text-[10px] uppercase tracking-[0.25em] text-charcoal/55">Currency<input value={form.currency} readOnly className={`${inputClass} cursor-not-allowed text-charcoal/50`} /></label></div></section>
      ...
      <section className={`${cardClass} rounded-[2rem] lg:col-start-1 lg:row-start-3`}>
        <p className="text-[10px] uppercase tracking-[0.3em] text-charcoal/45">Security</p><h2 className="mt-3 font-display text-4xl">Keep it yours.</h2>
        <div className="mt-8 space-y-5">
          <div className="border-b border-black/5 pb-5">
            <button type="button" onClick={() => setPasswordStep((current) => current === 'idle' ? 'details' : 'idle')} className="flex w-full items-center justify-between text-left text-sm hover:text-gold">Change password <span>{passwordStep === 'idle' ? '↗' : '×'}</span></button>
            {passwordStep === 'details' ? <div className="mt-4 grid gap-3"><input type="password" minLength={8} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} placeholder="New password" className="border-b border-black/15 bg-transparent py-2 text-sm outline-none focus:border-gold" /><input type="password" minLength={8} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Confirm new password" className="border-b border-black/15 bg-transparent py-2 text-sm outline-none focus:border-gold" /><button type="button" disabled={passwordBusy} onClick={requestPasswordChange} className="justify-self-start text-[10px] uppercase tracking-[0.16em] text-gold disabled:opacity-40">{passwordBusy ? 'Sending…' : 'Send verification code'}</button></div> : null}
            {passwordStep === 'otp' ? <div className="mt-4"><p className="text-xs leading-5 text-charcoal/50">Enter the code sent to your registered email. It expires in 10 minutes.</p><div className="mt-3 flex gap-3"><input value={passwordOtp} inputMode="numeric" onChange={(event) => setPasswordOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="6-digit code" className="min-w-0 flex-1 border-b border-black/15 bg-transparent py-2 text-sm tracking-[0.25em] outline-none focus:border-gold" /><button type="button" disabled={passwordBusy} onClick={verifyPasswordChange} className="text-[10px] uppercase tracking-[0.16em] text-gold disabled:opacity-40">{passwordBusy ? 'Verifying…' : 'Verify'}</button></div></div> : null}
          </div>
          <div className="border-b border-black/5 pb-5"><div className="flex items-center justify-between text-sm"><span>Change email</span><span className="text-xs text-charcoal/45">Verification required</span></div>{emailStep === 'idle' ? <div className="mt-4 flex gap-3"><input value={newEmail} onChange={(event) => setNewEmail(event.target.value)} placeholder="new@email.com" className="min-w-0 flex-1 border-b border-black/15 bg-transparent py-2 text-sm outline-none focus:border-gold" /><button type="button" onClick={requestEmailChange} className="text-[10px] uppercase tracking-[0.16em] text-gold">Send code</button></div> : <div className="mt-4 flex gap-3"><input value={emailOtp} onChange={(event) => setEmailOtp(event.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="6-digit code" className="min-w-0 flex-1 border-b border-black/15 bg-transparent py-2 text-sm tracking-[0.25em] outline-none focus:border-gold" /><button type="button" onClick={verifyEmailChange} className="text-[10px] uppercase tracking-[0.16em] text-gold">Verify</button></div>}</div>
        </div>
      </section>
    </div>
    <div className="mx-auto mt-5 flex max-w-6xl flex-col-reverse gap-4 sm:flex-row sm:items-center sm:justify-between"><button type="button" onClick={() => { setForm(saved); setErrors({}); }} disabled={!dirty || saving} className="text-[10px] uppercase tracking-[0.22em] text-charcoal/45 transition hover:text-charcoal disabled:cursor-not-allowed disabled:opacity-30">Reset form</button><div className="flex gap-3"><button type="button" onClick={() => { setForm(saved); setErrors({}); }} disabled={!dirty || saving} className="rounded-full border border-black/15 px-7 py-4 text-[10px] uppercase tracking-[0.2em] transition hover:border-charcoal disabled:cursor-not-allowed disabled:opacity-30">Cancel changes</button><button type="button" onClick={save} disabled={!dirty || saving} className="rounded-full bg-ink px-8 py-4 text-[10px] uppercase tracking-[0.2em] text-ivory transition hover:bg-gold disabled:cursor-not-allowed disabled:opacity-30">{saving ? 'Saving…' : 'Save changes'}</button></div></div>
    <div className="mx-auto mt-8 flex max-w-6xl justify-start border-t border-black/10 pt-6"><button type="button" onClick={logout} className="text-[10px] uppercase tracking-[0.22em] text-red-700/80 transition hover:text-red-700">Sign out</button></div>
    <div className="mx-auto mt-4 flex max-w-6xl justify-start"><button type="button" onClick={deleteAccount} className="text-[10px] uppercase tracking-[0.22em] text-red-700/80 transition hover:text-red-700">Delete account</button></div>
