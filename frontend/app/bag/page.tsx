...
import { Image as ImageIcon, ShoppingBag, X } from 'lucide-react';
...
export default function BagPage() {
  ...
  useEffect(() => {
    ...
    sync(); window.addEventListener(cartChangedEvent, sync); window.addEventListener('storage', sync);
    return () => { window.removeEventListener(cartChangedEvent, sync); window.removeEventListener('storage', sync); };
  }, []);

  if (signedIn === null) return null;

  return <main className="min-h-screen bg-ivory px-4 py-24 text-charcoal sm:px-8 lg:px-12"><div className="mx-auto max-w-6xl">
    ...
    {!signedIn ? <div className="py-16 text-center"><p className="text-sm text-charcoal/60">Your bag is connected to your RK account.</p><Link href="/account" className="mt-8 inline-flex bg-ink px-7 py-4 text-xs uppercase tracking-[0.28em] text-ivory transition hover:bg-gold hover:text-ink">Sign in</Link></div> : cart.items.length ? <div className="mt-10 grid gap-12 lg:grid-cols-[1fr_20rem]"><div>{cart.items.map((item) => <article key={`${item.productId}:${item.variant?.id || ''}`} className="grid grid-cols-[6rem_1fr_auto] gap-4 border-b border-black/10 py-5 dark:border-white/10 sm:grid-cols-[8rem_1fr_auto]"><div className="grid aspect-[3/4] place-items-center overflow-hidden bg-sand">{item.image ? <img src={item.image} alt={item.name} className="h-full w-full object-cover" /> : <ImageIcon size={20} className="text-charcoal/35" />}</div><div><p className="font-display text-xl sm:text-2xl">{item.name}</p>{item.variant ? <p className="mt-2 text-[0.58rem] uppercase tracking-[0.2em] text-charcoal/50">{item.variant.name}: {item.variant.value}</p> : null}<p className="mt-3 text-xs text-charcoal/65">Quantity {item.quantity}</p><p className="mt-2 text-sm">{inr.format(item.price)}</p></div><button type="button" onClick={() => removeStoredCartItem(item.productId, item.variant?.id)} aria-label={`Remove ${item.name}`} className="self-start p-2 text-charcoal/45 transition hover:text-gold"><X size={16} /></button></article>)}</div><aside className="h-fit border border-black/12 p-6 dark:border-white/15"><p className="text-[0.58rem] uppercase tracking-[0.28em] text-charcoal/50">Summary</p><div className="mt-6 flex justify-between border-b border-black/10 pb-5 text-sm dark:border-white/10"><span>Subtotal</span><span>{inr.format(getCartSubtotal(cart))}</span></div><p className="mt-5 text-xs leading-6 text-charcoal/55">Shipping and final order details are confirmed by the RK team.</p><button type="button" className="mt-7 w-full bg-ink px-5 py-4 text-[0.6rem] uppercase tracking-[0.25em] text-ivory">Continue</button></aside></div> : <div className="py-20 text-center"><p className="font-display text-3xl">Your bag is empty.</p><Link href="/collections" className="mt-8 inline-flex border-b border-charcoal/40 pb-2 text-xs uppercase tracking-[0.28em] transition hover:border-gold hover:text-gold">Explore collections</Link></div>}
  </div></main>;
}
