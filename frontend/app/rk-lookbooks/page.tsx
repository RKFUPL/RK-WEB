        ...
        <section className="mt-28 md:mt-40">
          ...
          <div ref={shelfRef} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} onWheel={handleShelfWheel} className={`rk-archive-shelf mt-12 flex gap-8 overflow-x-auto overscroll-x-contain pb-8 touch-pan-x select-none md:gap-10 ${isDragging ? 'is-dragging' : ''}`}>
            {lookbooks.map((lookbook, index) => {
              ...
              return <Link key={lookbook.title} href={lookbook.href ?? '/rk-lookbooks'} target={lookbook.href ? '_blank' : undefined} rel={lookbook.href ? 'noopener noreferrer' : undefined} onClick={(event) => { if (dragged.current) { event.preventDefault(); dragged.current = false; } }} className={`group w-[16rem] shrink-0 text-left md:w-[17rem] ${activeSpotlight?.title === lookbook.title ? 'border-t border-gold pt-4' : 'pt-4'}`}>
                <div className={`relative aspect-[3/4] w-full overflow-hidden rounded-[3px] ${lookbook.comingSoon ? 'border border-black/10' : 'bg-sand'}`}>
                  {lookbook.comingSoon ? <div className="flex h-full items-center justify-center text-center text-[0.62rem] uppercase tracking-[0.35em] text-charcoal/35">Coming<br />Soon</div> : cover ? <img src={cover} alt={`${lookbook.title} cover`} draggable={false} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03] group-hover:brightness-90" /> : null}
                  <span className="absolute left-4 top-4 font-display text-3xl text-white/80 drop-shadow">{String(index + 1).padStart(2, '0')}</span>
                </div>
                <h3 style={titleStyle(lookbook)} className="mt-6 min-h-[3.5rem] text-3xl leading-[0.88] text-charcoal transition-colors duration-300 group-hover:text-black">{lookbook.title}</h3>
                <p className="mt-3 text-[0.6rem] uppercase tracking-[0.32em] text-charcoal/45">{lookbook.comingSoon ? 'Coming soon' : 'SS26'}</p>
                <p className="mt-4 max-w-[16rem] text-sm leading-6 text-charcoal/60">{lookbook.description}</p>
                <span className="mt-6 inline-flex items-center gap-3 text-[0.6rem] uppercase tracking-[0.3em] text-charcoal/65 transition-opacity duration-300 md:opacity-0 md:group-hover:opacity-100">Read lookbook <ArrowRight className="h-3.5 w-3.5 transition-transform duration-300 group-hover:translate-x-1.5" /></span>
              </Link>;
            })}
          </div>
        </section>
