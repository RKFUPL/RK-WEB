...
import { formatDate, formatTime, relativeTime } from '@/lib/date-time';
...
type AudienceKind = DashboardData['visitorsList'][number]['kind'];

function audienceKindLabel(kind: AudienceKind) {
  return kind === 'admin' ? 'Admin' : kind === 'staff' ? 'Staff' : kind === 'customer' ? 'Customer' : 'Visitor';
}

function audienceDetailsLabel(kind: AudienceKind) {
  return kind === 'admin' || kind === 'staff' ? `${audienceKindLabel(kind)} session details` : `${audienceKindLabel(kind)} details`;
}

function audienceIdentity(kind: AudienceKind, key: string) {
  if (kind === 'admin' || kind === 'staff') return 'Authenticated internal session';
  if (kind === 'customer') return 'Registered customer';
  return `Anonymous ID …${key}`;
}
...
export default function AdminDashboard() {
  ...
  const loadDashboard = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      setData({
        ...payload,
        metrics: { ...payload.metrics, activeVisitors: payload.metrics?.activeVisitors ?? 0 },
        visitorsList: Array.isArray(payload.visitorsList) ? payload.visitorsList : [],
        newCustomersList: Array.isArray(payload.newCustomersList) ? payload.newCustomersList : [],
      } as DashboardData);
      setError('');
    } catch (requestError) {
  ...
  return <div className="space-y-5">
    ...
    <Card><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-[15px] font-semibold">Fulfillment overview</h2><p className="mt-1 text-xs text-[#8a9098]">Live order workload by shipment status.</p></div><a href="/admin/orders" className="text-[10px] uppercase tracking-[.14em] text-[#9a7a4d]">Manage orders</a></div><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">{([['processing', 'Processing'], ['packed', 'Packed'], ['shipped', 'Shipped'], ['out_for_delivery', 'Out for delivery'], ['delivered', 'Delivered'], ['return_requested', 'Returns'], ['cancelled', 'Cancelled']] as const).map(([status, label]) => <a key={status} href={`/admin/orders?fulfillment=${status}`} className="rounded-xl border border-black/[.06] p-4 transition hover:border-[#9a7a4d]/50 dark:border-white/[.08]"><span className="text-[9px] uppercase tracking-[.13em] text-[#8a9098]">{label}</span><span className="mt-2 block text-xl font-semibold">{data?.fulfillmentCounts?.[status] ?? 0}</span></a>)}</div></Card>
        ...
        <table className="w-full min-w-[720px] text-left">
          ...
          <tbody className="divide-y divide-black/[.06] dark:divide-white/[.08]">{visibleVisitors.map((visitor) => {
            ...
            return <tr key={`${visitor.key}-${visitor.lastSeen}`} role="button" tabIndex={0} onClick={() => setSelectedVisitor(visitor)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedVisitor(visitor); } }} className={`cursor-pointer text-xs transition hover:bg-[#faf8f4] focus:bg-[#faf8f4] focus:outline-none dark:hover:bg-white/[.03] dark:focus:bg-white/[.03] ${internal ? 'bg-emerald-50/35 dark:bg-emerald-500/[.04]' : ''}`}>
              ...
              <td className="max-w-[190px] truncate py-3 text-[#6e747d]" title={visitor.lastPath}>{visitor.lastPath}</td><td className="py-3 capitalize text-[#6e747d]">{visitor.source}</td><td className="py-3"><div className="flex items-center gap-2 text-[#6e747d]"><DeviceIcon size={15} /><span>{visitor.device} · {visitor.browser}</span></div></td><td className="py-3 text-[#6e747d]">{internal ? 'Dashboard session' : `${visitor.views} views · ${visitor.pages} pages`}</td><td className="py-3 text-right text-[#9298a0]">{visitor.current ? <span className="text-emerald-600">Current device · Live now</span> : visitor.active ? <span className="text-emerald-600">Live now</span> : relativeTime(visitor.lastSeen)}</td>
            </tr>;
          })}</tbody>
        </table>
    ...
    <Card className="bg-[#27231f] text-[#f8f3eb] dark:bg-[#302a24]"><h2 className="text-[15px] font-semibold">Data coverage</h2><p className="mt-2 text-sm leading-6 text-white/60">Traffic is collected from storefront page views. Revenue, orders, best sellers, stock, customers, wishlists, and ratings are calculated from their corresponding backend records and events—no dashboard value is hardcoded.</p>{data ? <p className="mt-3 text-[10px] uppercase tracking-[.16em] text-white/35">Last updated {formatTime(data.generatedAt)}</p> : null}</Card>

    {selectedVisitor ? <div className="fixed inset-0 z-[100] grid place-items-center bg-black/45 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedVisitor(null); }}><section role="dialog" aria-modal="true" aria-labelledby="visitor-details-title" className="w-full max-w-md rounded-2xl border border-black/10 bg-white p-6 text-[#20242b] shadow-2xl dark:border-white/10 dark:bg-[#191a1f] dark:text-white"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[.18em] text-[#9a7a4d]">{audienceDetailsLabel(selectedVisitor.kind)}</p><h2 id="visitor-details-title" className="mt-2 text-xl font-semibold">{selectedVisitor.label}</h2><p className="mt-1 text-xs text-[#9298a0]">{audienceIdentity(selectedVisitor.kind, selectedVisitor.key)}</p></div><button type="button" onClick={() => setSelectedVisitor(null)} aria-label="Close visitor details" className="rounded-lg p-2 transition hover:bg-black/5 dark:hover:bg-white/10"><X size={18} /></button></div><div className="mt-6 grid grid-cols-2 gap-3 text-sm">{[['Type', audienceKindLabel(selectedVisitor.kind)], ['Status', selectedVisitor.current ? 'Current device · Live now' : selectedVisitor.active ? 'Live now' : 'Inactive'], ['Source', selectedVisitor.source], ['Device', selectedVisitor.device], ['Browser', selectedVisitor.browser], ['Operating system', selectedVisitor.os], ['Last page', selectedVisitor.lastPath], ['Views', String(selectedVisitor.views)], ['Pages', String(selectedVisitor.pages)], ['Sessions', String(selectedVisitor.sessions)], ['Last seen', relativeTime(selectedVisitor.lastSeen)]].map(([label, value]) => <div key={label} className="rounded-xl bg-[#f7f7f5] p-3 dark:bg-white/[.05]"><p className="text-[10px] uppercase tracking-[.12em] text-[#9298a0]">{label}</p><p className="mt-1 truncate capitalize">{value}</p></div>)}</div><p className="mt-5 text-xs leading-5 text-[#9298a0]">{selectedVisitor.kind === 'admin' || selectedVisitor.kind === 'staff' ? 'This authenticated internal session is shown separately and excluded from storefront visitor totals.' : 'Anonymous visitors never expose names, email addresses, IP addresses, or other sensitive identity data.'}</p></section></div> : null}
    {audiencePopup ? <div className="fixed inset-0 z-[100] grid place-items-center bg-black/45 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setAudiencePopup(null); }}><section role="dialog" aria-modal="true" aria-labelledby="audience-popup-title" className="w-full max-w-lg rounded-2xl border border-black/10 bg-white p-6 text-[#20242b] shadow-2xl dark:border-white/10 dark:bg-[#191a1f] dark:text-white"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[.18em] text-[#9a7a4d]">Audience</p><h2 id="audience-popup-title" className="mt-2 text-xl font-semibold">{audiencePopup === 'customers' ? 'New customers' : 'Unique visitors'}</h2><p className="mt-1 text-xs text-[#9298a0]">{audiencePopup === 'customers' ? 'Customers registered during the selected period.' : 'Distinct storefront visitors during the selected period.'}</p></div><button type="button" onClick={() => setAudiencePopup(null)} aria-label="Close audience popup" className="rounded-lg p-2 transition hover:bg-black/5 dark:hover:bg-white/10"><X size={18} /></button></div>{audiencePopup === 'customers' ? customerData.length ? <div className="mt-6 max-h-[55vh] divide-y divide-black/[.06] overflow-y-auto dark:divide-white/[.08]">{customerData.map((customer) => <div key={customer.id} className="py-4"><p className="text-sm font-medium">{customer.name}</p><p className="mt-1 text-xs text-[#9298a0]">{customer.email || (customer.username ? `@${customer.username}` : 'No contact details')}</p><p className="mt-1 text-[10px] text-[#b0b4ba]">Joined {customer.createdAt ? formatDate(customer.createdAt) : 'recently'}</p></div>)}</div> : <Empty title="No new customers" copy="New registrations will appear here for the selected period." /> : visitorData.length ? <div className="mt-6 max-h-[55vh] divide-y divide-black/[.06] overflow-y-auto dark:divide-white/[.08]">{visitorData.map((visitor) => <button type="button" key={`${visitor.key}-${visitor.lastSeen}`} onClick={() => { setAudiencePopup(null); setSelectedVisitor(visitor); }} className="block w-full py-4 text-left transition hover:bg-[#faf8f4] dark:hover:bg-white/[.03]"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-medium">{visitor.label}<span className="ml-2 text-[10px] font-normal uppercase tracking-[.12em] text-[#9a7a4d]">{visitor.kind}</span></p><p className="mt-1 text-xs text-[#9298a0]">{visitor.lastPath} · {visitor.views} views · {visitor.source}</p></div><span className="whitespace-nowrap text-[10px] text-[#9298a0]">{visitor.current ? 'Current device' : visitor.active ? 'Live now' : relativeTime(visitor.lastSeen)}</span></div></button>)}</div> : <Empty title="No visitors" copy="Storefront visitors will appear here after they open a page." />}</section></div> : null}
  </div>;
}
