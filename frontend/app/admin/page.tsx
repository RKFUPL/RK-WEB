...
export default function AdminDashboard() {
  ...
  return <div className="space-y-5">
    ...
    <Card><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-[15px] font-semibold">Fulfillment overview</h2><p className="mt-1 text-xs text-[#8a9098]">Live order workload by shipment status.</p></div><a href="/admin/orders" className="text-[10px] uppercase tracking-[.14em] text-[#9a7a4d]">Manage orders</a></div><div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">{([['processing', 'Processing'], ['packed', 'Packed'], ['shipped', 'Shipped'], ['out_for_delivery', 'Out for delivery'], ['delivered', 'Delivered'], ['returns', 'Returns'], ['cancelled', 'Cancelled']] as const).map(([status, label]) => <a key={status} href={`/admin/orders?fulfillment=${status}`} className="rounded-xl border border-black/[.06] p-4 transition hover:border-[#9a7a4d]/50 dark:border-white/[.08]"><span className="text-[9px] uppercase tracking-[.13em] text-[#8a9098]">{label}</span><span className="mt-2 block text-xl font-semibold">{data?.fulfillmentCounts?.[status] ?? 0}</span></a>)}</div></Card>
