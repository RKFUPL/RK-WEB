  ...
  return <div className="space-y-5">
    ...
    <div ref={visitorPanelRef} className="scroll-mt-24"><Card>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-[10px] font-medium uppercase tracking-[.12em] text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{metrics ? `${metrics.activeVisitors} live · ${visitorData.filter((visitor) => visitor.current).length} current device` : 'Loading'}</span>
      </div>
