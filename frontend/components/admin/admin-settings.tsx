...
const defaults: SettingsForm = { storeName: 'Rashi Kapoor', supportEmail: '', currency: 'INR', timezone: 'Asia/Kolkata', orderPrefix: 'RK', lowStockThreshold: 5, analyticsRetentionDays: 365, emailNotifications: true, orderNotifications: true, lowStockNotifications: true, courierOptions: [] };
...
export function AdminSettings() {
  ...
  return <div className="mt-10 space-y-5">
    ...
    <section className="rounded-2xl border border-black/[.06] bg-white p-6 dark:border-white/[.08] dark:bg-[#191a1f]"><div className="flex items-center gap-3"><Store size={19} className="text-[#9a7a4d]" /><div><h2 className="text-base font-semibold">Shipment couriers</h2><p className="mt-1 text-xs text-[#8a9098]">Configure the courier choices shown when staff ship an order. One name per line.</p></div></div><label className="mt-7 block text-[10px] uppercase tracking-[.18em] text-[#8a9098]">Courier options<textarea rows={4} value={form.courierOptions.join('\n')} onChange={(event) => update('courierOptions', event.target.value.split(/\r?\n|,/).map((value) => value.trim()).filter(Boolean))} placeholder={'Delhivery\nBlue Dart\nIndia Post'} className={inputClass} /></label></section>
