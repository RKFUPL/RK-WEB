...
const columns: Record<Section, Array<[string, string]>> = {
  products: [['name', 'Product'], ['sku', 'SKU'], ['price', 'Price'], ['stock', 'Stock'], ['sizeInventory', 'Size inventory']],
...
function SizeInventoryEditor({ form, onChange }: { form: Record<string, string>; onChange: (next: Record<string, string>) => void }) {
  let entries: Array<{ size: string; stock: number; enabled: boolean }> = [];
  try { entries = JSON.parse(form.sizeInventory || '[]') as Array<{ size: string; stock: number; enabled: boolean }>; } catch { entries = []; }
  const enabled = form.sizeSystemEnabled === 'true' || entries.length > 0;
  const stockFor = (size: string) => Number(entries.find((entry) => entry.size === size)?.stock ?? 0);
  const setEnabled = (value: boolean) => onChange({ ...form, sizeSystemEnabled: String(value), sizeInventory: value ? JSON.stringify(standardSizes.map((size) => ({ size, stock: stockFor(size), enabled: true }))) : '[]' });
  const setStock = (size: string, value: string) => {
    const next = standardSizes.map((entrySize) => ({ size: entrySize, stock: entrySize === size ? Math.max(0, Number(value) || 0) : stockFor(entrySize), enabled: true }));
    onChange({ ...form, sizeSystemEnabled: 'true', sizeInventory: JSON.stringify(next) });
  };
  return <div className="mt-6 rounded-xl border border-[#9a7a4d]/20 bg-[#faf8f4] p-5 dark:bg-white/[.03]"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[.16em] text-[#858b94]">Size inventory</p><p className="mt-1 text-xs text-[#858b94]">Allocate stock by size when this product uses XS–XL inventory.</p></div><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={enabled} onChange={(event) => setEnabled(event.target.checked)} /> Enable sizes</label></div>{enabled ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-5">{standardSizes.map((size) => <label key={size} className="text-[10px] uppercase tracking-[.14em] text-[#858b94]">{size}<input type="number" min={0} value={stockFor(size)} onChange={(event) => setStock(size, event.target.value)} className={`${inputClass} mt-2 normal-case tracking-normal`} /></label>)}</div> : null}<label className="mt-4 block text-[10px] uppercase tracking-[.14em] text-[#858b94]">Unallocated stock<input type="number" min={0} value={form.unallocatedStock ?? '0'} onChange={(event) => onChange({ ...form, unallocatedStock: event.target.value })} className={`${inputClass} mt-2 normal-case tracking-normal`} /></label></div>;
}
...
function formatValue(key: string, value: unknown) {
  ...
  if (key === 'sizeInventory' && Array.isArray(value)) return value.length ? value.map((entry) => `${String((entry as Record<string, unknown>).size)}: ${String((entry as Record<string, unknown>).stock ?? 0)}`).join(' · ') : 'Legacy / unallocated';
...
export function OperationsSection({ section: rawSection }: { section: string }) {
  ...
  const beginEdit = (item: Item) => {
    ...
    setEditing(item.id);
    setEditForm({ ...values, availability, stock: String(item.stock ?? '0'), sizeInventory: JSON.stringify(item.sizeInventory ?? []), sizeSystemEnabled: String(Boolean(item.sizeSystemEnabled || (Array.isArray(item.sizeInventory) && item.sizeInventory.length))), unallocatedStock: String(item.unallocatedStock ?? 0), changeReason: '' });
  };
  ...
  return <section className="mt-10 rounded-2xl border border-black/[.06] bg-white p-6 shadow-[0_4px_20px_rgba(25,31,38,.035)] dark:border-white/[.08] dark:bg-[#191a1f] md:p-8">
    ...
    {section === 'products' && editing ? <SizeInventoryEditor form={editForm} onChange={setEditForm} /> : null}
