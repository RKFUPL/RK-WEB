...
export function OperationsSection({ section: rawSection }: { section: string }) {
  ...
  const editableFields = useMemo(() => section === 'products' ? ['name', 'sku', 'price', 'stock', 'status', 'availability', 'description'] : section === 'customers' ? ['displayName', 'email', 'phone'] : section === 'orders' ? ['customerName', 'email', 'total'] : section === 'quotes' ? ['customerName', 'email', 'total', 'notes'] : [], [section]);
  const beginEdit = (item: Item) => { setEditing(item.id); setEditForm({ ...Object.fromEntries(editableFields.map((key) => [key, String(item[key] ?? '')])), changeReason: '' }); };
