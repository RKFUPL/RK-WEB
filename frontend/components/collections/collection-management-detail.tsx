...
function ProductDrawer({ product, collection, collections, canManageProducts, canManageInventory, busy, onClose, onReload, onRemove }: {
  ...
  const details: Array<[string, unknown]> = [
    ...
    ['Availability', availabilityLabels[product.availability]], ['Quantity', product.stock], ['Stock status', product.stock && product.stock > 0 ? 'Available' : product.availability === 'custom_order' ? 'Made to order' : 'No stock'],
...
export function CollectionManagementDetail({ slug, basePath }: { slug: string; basePath: '/admin/collections' | '/staff/collections' }) {
  ...
  const [collectionForm, setCollectionForm] = useState({ name: '', status: 'collection', description: '', heroImage: '' });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setCollectionForm({ name: collection.name, status: collection.status, description: collection.description ?? '', heroImage: collection.heroImage ?? '' });
  ...
  const saveCollection = async (event: FormEvent) => {
    ...
    const payload = await relationRequest(`/api/staff/collections/${encodeURIComponent(slug)}`, { method: 'PATCH', body: JSON.stringify(collectionForm) });
  ...
  return <><section className="mt-8"><Link href={basePath} className="inline-flex items-center gap-2 text-xs text-[#7d838d] transition hover:text-[#9a7a4d]"><ArrowLeft size={14} />Back to collections</Link><div className="mt-5 overflow-hidden rounded-2xl border border-black/[.06] bg-white shadow-[0_4px_20px_rgba(25,31,38,.035)] dark:border-white/[.08] dark:bg-[#191a1f]"><div className="grid lg:grid-cols-[1fr_20rem]"><div className="p-6 sm:p-8"><p className="text-[10px] uppercase tracking-[.2em] text-[#9a7a4d]">Collection management</p><div className="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-3xl font-semibold">{collection.name}</h2><p className="mt-2 text-xs text-[#8a9098]">{collection.slug}</p></div>{canManageProducts ? <button type="button" onClick={() => setEditingCollection((current) => !current)} className="rounded-lg border border-black/10 px-4 py-2.5 text-xs transition hover:border-[#9a7a4d]">Edit collection</button> : null}</div><div className="mt-7 grid grid-cols-2 gap-5 sm:grid-cols-3"><div><p className="text-[9px] uppercase tracking-[.15em] text-[#8a9098]">Status</p><p className="mt-1 text-sm capitalize">{statusLabel(collection.status)}</p></div><div><p className="text-[9px] uppercase tracking-[.15em] text-[#8a9098]">Products</p><p className="mt-1 text-sm">{collection.productCount}</p></div><div className="col-span-2 sm:col-span-1"><p className="text-[9px] uppercase tracking-[.15em] text-[#8a9098]">Created</p><p className="mt-1 text-sm">{collection.createdAt ? new Date(collection.createdAt).toLocaleDateString() : '—'}</p></div></div><p className="mt-7 max-w-3xl text-sm leading-7 text-[#6e747d]">{collection.description || 'No collection description yet.'}</p></div><div className="min-h-56 border-t border-black/[.06] bg-[#efece6] dark:border-white/[.08] dark:bg-white/[.03] lg:border-l lg:border-t-0">{collection.heroImage ? <img src={collection.heroImage} alt={`${collection.name} collection`} className="h-full max-h-80 w-full object-cover lg:max-h-none" /> : <div className="grid h-full min-h-56 place-items-center text-center text-[#9a7a4d]"><div><ImageIcon size={28} className="mx-auto" /><p className="mt-2 text-[10px] uppercase tracking-[.16em]">Hero placeholder</p></div></div>}</div></div>
    {editingCollection ? <form onSubmit={saveCollection} className="border-t border-black/[.06] bg-[#faf8f4] p-6 dark:border-white/[.08] dark:bg-white/[.025]"><div className="grid gap-4 md:grid-cols-2"><label className="text-[9px] uppercase tracking-[.14em] text-[#8a9098]">Name<input required value={collectionForm.name} onChange={(event) => setCollectionForm((current) => ({ ...current, name: event.target.value }))} className={inputClass} /></label><label className="text-[9px] uppercase tracking-[.14em] text-[#8a9098]">Status<select value={collectionForm.status} onChange={(event) => setCollectionForm((current) => ({ ...current, status: event.target.value }))} className={inputClass}><option value="collection">Collection</option><option value="draft">Draft</option><option value="archived">Archived</option></select></label><label className="text-[9px] uppercase tracking-[.14em] text-[#8a9098] md:col-span-2">Description<textarea rows={3} value={collectionForm.description} onChange={(event) => setCollectionForm((current) => ({ ...current, description: event.target.value }))} className={inputClass} /></label><label className="text-[9px] uppercase tracking-[.14em] text-[#8a9098] md:col-span-2">Cloudinary / hero image URL<input value={collectionForm.heroImage} onChange={(event) => setCollectionForm((current) => ({ ...current, heroImage: event.target.value }))} className={inputClass} /></label></div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setEditingCollection(false)} className="rounded-lg border border-black/10 px-4 py-2 text-xs">Cancel</button><button disabled={busy} className="rounded-lg bg-[#24211e] px-4 py-2 text-xs text-white disabled:opacity-40">Save collection</button></div></form> : null}
  </div></section>
