  ...
  async function pay() {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/api/payments/razorpay/order`, {
        ...
        body: JSON.stringify({ checkoutAttemptId, cart: cart.items.map((item) => ({ productId: item.productId, quantity: item.quantity, variant: item.variant })), shipping: { ...shipping, addressId: selectedAddressId || undefined } }),
      });
