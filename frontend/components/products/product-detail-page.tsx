...
export function ProductDetailPage({ productId }: { productId: string }) {
  ...
  const sizes = sizeInventory.length ? sizeInventory.filter((item) => item.enabled !== false).map((item) => item.size) : product.attributes?.sizes ?? [];
  const selectedSizeEntry = sizeInventory.find((item) => item.size === selectedSize);
  const customSizeFields = product.customSizeConfig?.fields ?? [];
  ...
  const addToCart = () => {
    if (!requireSignIn()) return;
    if (sizes.length && !selectedSize && !customMeasurements._customReady) { setMessage('Please select a size or enter custom measurements.'); return; }
  ...
  return <main className="bg-ivory text-charcoal"><StickyHeader />
    <section className="mx-auto max-w-[100rem] px-4 pb-20 pt-28 sm:px-8 lg:px-12 lg:pb-28 lg:pt-32">
      ...
      <div className="grid gap-10 lg:grid-cols-[1.16fr_0.84fr] lg:gap-16 xl:gap-24">
        ...
        <div className="lg:sticky lg:top-32 lg:self-start">
          ...
          {sizes.length ? <fieldset className="mt-8"><legend className="text-[0.58rem] uppercase tracking-[0.28em] text-charcoal/50">Select size</legend><div className="mt-4 flex flex-wrap gap-2">{sizes.map((size) => { const item = sizeInventory.find((entry) => entry.size === size); const unavailable = product.availability === 'sold_out' || (product.availability === 'in_stock' && item && item.stock <= 0); return <button key={size} type="button" disabled={unavailable} onClick={() => { setSelectedSize(size); setCustomMeasurements({}); setQuantity(1); setMessage(''); }} className={`min-w-12 border px-4 py-3 text-xs transition ${selectedSize === size ? 'border-gold bg-gold text-ink' : 'border-black/15 hover:border-gold dark:border-white/20'} ${unavailable ? 'cursor-not-allowed opacity-30' : ''}`}>{size}</button>; })}</div>{product.availability === 'in_stock' && selectedSizeEntry ? <p className="mt-3 text-xs text-charcoal/55">{selectedSizeEntry.stock} available in {selectedSize}.</p> : null}{customSizeFields.length ? <button type="button" onClick={() => setCustomSizeOpen(true)} className="mt-4 inline-flex items-center gap-2 border-b border-charcoal/30 pb-2 text-[0.58rem] uppercase tracking-[0.22em] text-charcoal/70 transition hover:border-gold hover:text-gold"><Ruler size={14} />Request custom size</button> : null}</fieldset> : customSizeFields.length ? <button type="button" onClick={() => setCustomSizeOpen(true)} className="mt-8 inline-flex items-center gap-2 border-b border-charcoal/30 pb-2 text-[0.58rem] uppercase tracking-[0.22em] text-charcoal/70 transition hover:border-gold hover:text-gold"><Ruler size={14} />Request custom size</button> : null}
          {customMeasurements._customReady ? <p className="mt-3 text-xs text-gold">Custom measurements added to this piece.</p> : null}
          <div className="mt-8 flex items-center justify-between border-y border-black/12 py-4 dark:border-white/15"><span className="text-[0.58rem] uppercase tracking-[0.28em] text-charcoal/50">Quantity</span><div className="flex items-center gap-4"><button type="button" onClick={() => setQuantity((value) => Math.max(1, value - 1))} aria-label="Decrease quantity" className="grid h-8 w-8 place-items-center border border-black/15 transition hover:border-gold dark:border-white/20"><Minus size={13} /></button><span className="min-w-5 text-center text-sm">{quantity}</span><button type="button" disabled={Boolean(selectedSizeEntry && product.availability === 'in_stock' && quantity >= selectedSizeEntry.stock)} onClick={() => setQuantity((value) => value + 1)} aria-label="Increase quantity" className="grid h-8 w-8 place-items-center border border-black/15 transition hover:border-gold disabled:cursor-not-allowed disabled:opacity-35 dark:border-white/20"><Plus size={13} /></button></div></div>
    ...
    <Footer />
    <AnimatePresence>{customSizeOpen ? <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] grid place-items-center bg-ink/65 p-4" onClick={() => setCustomSizeOpen(false)}><motion.div initial={{ y: 18, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 18, opacity: 0 }} onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-label="Enter custom measurements" className="max-h-[90vh] w-full max-w-lg overflow-y-auto bg-ivory p-6 text-charcoal shadow-2xl sm:p-8"><div className="flex items-start justify-between gap-5"><div><p className="text-[0.58rem] uppercase tracking-[0.28em] text-gold">Made for you</p><h2 className="mt-3 font-display text-3xl">Custom measurements</h2></div><button type="button" onClick={() => setCustomSizeOpen(false)} aria-label="Close custom measurements"><X size={19} /></button></div><p className="mt-4 text-sm leading-6 text-charcoal/60">Enter the measurements requested for this piece. They will remain attached to your order for the RK team.</p><label className="mt-6 block text-[0.58rem] uppercase tracking-[0.22em] text-charcoal/50">Unit<select value={customSizeUnit} onChange={(event) => setCustomSizeUnit(event.target.value as 'cm' | 'in')} className="mt-2 w-full border-b border-black/15 bg-transparent py-3 text-sm outline-none"><option value="cm">Centimetres (cm)</option><option value="in">Inches (in)</option></select></label><div className="mt-5 grid gap-5 sm:grid-cols-2">{customSizeFields.map((field) => <label key={field} className="text-[0.58rem] uppercase tracking-[0.18em] text-charcoal/50">{field}<input inputMode="decimal" value={customMeasurements[field] ?? ''} onChange={(event) => setCustomMeasurements((current) => ({ ...current, [field]: event.target.value, _customReady: '' }))} className="mt-2 w-full border-b border-black/15 bg-transparent py-3 text-sm normal-case tracking-normal outline-none" /></label>)}</div><button type="button" onClick={() => { if (customSizeFields.some((field) => !customMeasurements[field]?.trim())) { setMessage('Complete every custom measurement.'); return; } setSelectedSize(''); setCustomMeasurements((current) => ({ ...current, _customReady: 'true' })); setCustomSizeOpen(false); setMessage('Custom measurements saved.'); }} className="mt-8 inline-flex w-full items-center justify-center gap-2 bg-ink px-5 py-4 text-[0.6rem] uppercase tracking-[0.24em] text-ivory transition hover:bg-gold hover:text-ink"><Check size={14} />Use these measurements</button></motion.div></motion.div> : null}</AnimatePresence>
