...
export function AdminOrderDetail({ orderId }: { orderId: string }) {
  ...
  return <section className="mt-10 rounded-2xl border border-black/[.06] bg-white p-5 shadow-[0_4px_20px_rgba(25,31,38,.035)] dark:border-white/[.08] dark:bg-[#191a1f] sm:p-8">
    ...
    <div className="mt-7 grid gap-4 md:grid-cols-2"><section className={panelClass}><p className="text-[9px] uppercase tracking-[.18em] text-[#9a7a4d]">Customer</p><p className="mt-4 font-medium">{order.customerName || 'Customer'}</p><p className="mt-2 text-xs leading-6 text-[#858b94]">{order.email}<br />{order.phone || 'No phone recorded'}</p></section><section className={panelClass}><p className="text-[9px] uppercase tracking-[.18em] text-[#9a7a4d]">Shipping address</p><p className="mt-4 text-xs leading-6 text-[#6e747d]">{address.fullName}<br />{address.line1}{address.line2 ? `, ${address.line2}` : ''}<br />{address.city}, {address.state} {address.postalCode}<br />{address.country}</p></section></div>
    <section className={`${panelClass} mt-4`}><p className="text-[9px] uppercase tracking-[.18em] text-[#9a7a4d]">Products</p><div className="mt-4 divide-y divide-black/[.07] dark:divide-white/[.07]">{order.items.map((item, index) => <div key={`${item.productId || item.sku || item.name}-${index}`} className="grid gap-4 py-4 sm:grid-cols-[5rem_1fr_auto]"><div className="h-20 overflow-hidden bg-[#ece3d4]">{item.image ? <img src={item.image} alt="" className="h-full w-full object-cover" /> : null}</div><div><p className="font-medium">{item.name}</p><p className="mt-1 text-xs text-[#858b94]">SKU {item.sku || '—'} · Qty {item.quantity}{item.size ? ` · Size ${item.size}` : ''}{item.variant?.value ? ` · ${item.variant.value}` : ''}{item.purchaseMode ? ` · ${item.purchaseMode === 'custom_size' ? 'Custom size order' : 'Standard size order'}` : ''}</p>{item.customSize?.measurements ? <p className="mt-2 text-[11px] text-[#858b94]">Custom size ({item.customSize.unit || 'cm'}): {Object.entries(item.customSize.measurements).map(([key, value]) => `${key} ${value}`).join(' · ')}</p> : null}</div><p className="text-sm">{inr.format(item.lineTotal || (item.unitPrice || 0) * item.quantity)}</p></div>)}</div></section>
