...
export function CustomerOrderDetail({ orderId }: { orderId: string }) {
  ...
  return <main className="min-h-screen bg-ivory text-charcoal"><StickyHeader /><section className="mx-auto max-w-7xl px-5 pb-24 pt-36 sm:px-8 lg:px-12 lg:pt-44"><Link href="/account/orders" className="inline-flex items-center gap-3 text-[0.58rem] uppercase tracking-[0.24em] text-charcoal/55 transition hover:text-gold"><ArrowLeft size={15} /> My orders</Link>{error ? <p role="alert" className="mt-7 border border-red-300 bg-red-50 px-5 py-4 text-sm text-red-700 dark:bg-red-950/20">{error}</p> : null}{loading ? <div className="mt-10 h-[38rem] animate-pulse border border-black/10 bg-black/[.025] dark:border-white/10" /> : order ? <>
    ...
    <div className="mt-10 grid gap-6 lg:grid-cols-[0.78fr_1.22fr]"><section className={`${panel} lg:row-span-2`}><p className="text-[0.55rem] uppercase tracking-[0.25em] text-gold">Order status</p><h2 className="mt-3 font-display text-3xl">From the house to you.</h2><div className="mt-8"><OrderTimeline order={order} /></div></section><section className={panel}>
      <p className="text-[0.55rem] uppercase tracking-[0.25em] text-gold">Items</p>
      <div className="mt-6 divide-y divide-black/10 dark:divide-white/10">{order.items.map((item, index) => <div key={`${item.productId || item.name}-${index}`} className="grid grid-cols-[5rem_1fr_auto] gap-4 py-5 first:pt-0">
        <div className="h-24 overflow-hidden rounded-[12px] bg-sand">{item.image ? <img src={item.image} alt={`${item.name} in ${item.colour || item.variant?.value || 'selected colour'}`} className="h-full w-full object-cover" /> : null}</div>
        <div><p className="font-display text-xl">{item.name}</p><p className="mt-1 text-[0.6rem] uppercase tracking-[0.16em] text-charcoal/45">{item.sku || 'RK piece'}</p><p className="mt-2 text-xs text-charcoal/55">{item.collection || 'RK collection'} · {item.colour || item.variant?.value || 'Selected colour'}</p>{item.size ? <p className="mt-1 text-xs text-charcoal/55">Size {item.size}</p> : null}<p className="mt-1 text-xs text-charcoal/55">Quantity {item.quantity}</p>{item.customSize?.measurements ? <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-charcoal/55">{Object.entries(item.customSize.measurements).map(([name, value]) => <div key={name} className="contents"><dt>{name}</dt><dd>{value} {item.customSize?.unit || 'cm'}</dd></div>)}</dl> : null}</div>
        <p className="text-sm">{inr.format((item.unitPrice || 0) * item.quantity)}</p>
      </div>)}</div>
      <div className="mt-6 flex justify-between border-t border-black/10 pt-5 font-display text-2xl dark:border-white/10"><span>Total</span><span>{inr.format(order.total || 0)}</span></div>
    </section>
